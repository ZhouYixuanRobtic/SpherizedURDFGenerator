...
Three named presets are available: \emph{single} (one conservative capsule per link), \emph{default} (recommended low-count fit), and \emph{high\_detail} (more axial sections and larger capsule budget).

% ---- C. Python and CLI Interface ----
\subsection{Interface}

The toolchain exposes three core operations through both a command-line interface and a Python API: generation (producing URDF output for any mode), validation (computing coverage and tightness metrics from generated output), and comparison (evaluating quality differences between configurations). The Python API additionally supports batch processing across all modes and presets, and a containerized pipeline ensures reproducible execution across environments.
