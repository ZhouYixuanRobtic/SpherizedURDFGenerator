...
For sphere and capsule modes, the toolchain defaults to fitting the visual mesh to capture the true link shape, since visual meshes typically reflect the actual link geometry while collision meshes may be pre-simplified.
A flag switches the fit source to the collision mesh when visual meshes are unavailable.
...
For sphere and capsule modes, a JSON sidecar file (at the same path as the output URDF) stores the canonical analytic parameters.
...
Xacro files must be preprocessed to standard URDF before use (via the xacro macro processor).
The only accepted and emitted format is URDF.
...
The output is a standard URDF whose mesh elements point to generated OBJ convex-hull files.
...
Two presets are provided: \emph{single} (one bounding sphere per link) and \emph{default} (multi-sphere medial tree).
...
Three named presets are available: \emph{single} (one coarse capsule per link), \emph{default} (recommended low-count fit), and \emph{high\_detail} (more axial sections and larger capsule budget).

% ---- C. Python and CLI Interface ----
\subsection{Interface}

The toolchain exposes three core operations through both a command-line interface and a Python API: generation (producing URDF output for any mode), validation (computing coverage and tightness metrics from generated output), and comparison (evaluating quality differences between configurations). The Python API additionally supports batch processing across all modes and presets. The compare-all workflow generates robot\_viewer~\cite{robot_viewer} visualization bundles for qualitative inspection across all modes and presets. A containerized pipeline ensures reproducible execution across environments.
...
The capsule fitting pipeline uses a fixed random seed for stochastic sub-steps such as the Welzl minimum-enclosing-circle shuffle and k-means initialization, ensuring bit-identical output for identical inputs and configuration.
Built-in validation gates and comparison workflows allow users to verify that approximation quality meets specified thresholds.
