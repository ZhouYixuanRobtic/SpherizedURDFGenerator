...
For the default visual-mesh source, any non-OBJ/STL visual meshes (e.g., COLLADA \texttt{.dae} files) are converted to OBJ format in a Python preprocessing step using Trimesh~\cite{trimesh}.
...
The section count $N$ is controlled by the parameter \texttt{NSections}; typical values range from 2 (coarse) to 8 (fine).
...
When \texttt{AdaptiveCircleCount} is enabled, the pipeline starts with one MEC and iteratively refines it using Lloyd-style clustering.
...
When \texttt{AdaptiveCircleCount} is disabled (the default), a fixed number of circles $k$ (controlled by \texttt{MaxCirclesPerSection}) is fitted per section using deterministic farthest-point initialization followed by $k$-means clustering.
...
A split is accepted only if it reduces the capsule union volume or improves the radius-inflation metric above a threshold (\texttt{MinSplitVolumeImprovement}).
Splitting is also triggered when the capsule union volume relative to the link AABB volume (\texttt{capV/aabb}) exceeds a configured ceiling (\texttt{MaxCapVAabbRatio}).
...
If after refinement the capsule count exceeds $\texttt{MaxCapsulesPerLink}$, the pipeline iteratively removes the capsule whose removal minimizes the resulting capsule union volume, re-growing the remaining capsules to maintain coverage after each removal.
