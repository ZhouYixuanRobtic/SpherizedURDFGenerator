...
The goal is to find a set of capsules $\mathcal{C} = \{c_1, \dots, c_k\}$ such that every mesh vertex lies inside at least one capsule (\Cref{eq:coverage}), the capsule count does not exceed a per-link budget $K_{\max}$, and excess volume and radius inflation are minimized.
\begin{equation}
\forall \mathbf{v} \in V,\; \min_{c \in \mathcal{C}} d(\mathbf{v}, c) \leq 0. \label{eq:coverage}
\end{equation}
...
Approximately collinear adjacent capsules (axial dot product $> 0.3$, radius difference $< 15\%$) are merged into a single capsule spanning the full extent. Each vertex is assigned to its nearest capsule by point-to-segment distance, and radii are expanded to cover all assigned vertices, ensuring \Cref{eq:coverage}.
