...
The practical objective is to find a compact set of capsules $\mathcal{C} = \{c_1, \dots, c_k\}$ under a per-link budget $K_{\max}$ while reducing uncovered sample distance and excess volume. We use mesh vertices as the primary validation samples unless otherwise stated:
\begin{equation}
\max_{\mathbf{v} \in V} \min_{c \in \mathcal{C}} d(\mathbf{v}, c) \leq \epsilon .
\label{eq:vertex_coverage}
\end{equation}
This is a vertex-sample coverage criterion, not a proof that every point on every triangular face is enclosed. Surface-sampled coverage is therefore treated as a separate validation target when conservative collision claims are required.
...
Before fitting, the pipeline resolves the mesh source from the URDF. Non-OBJ/STL visual meshes are converted to OBJ format using Trimesh~\cite{trimesh}, then processed through ManifoldPlus~\cite{huang2022manifoldplus} to obtain a watertight manifold required for robust cross-section slicing. The mesh is transformed from the mesh-local frame to the link frame using the URDF origin transform ($\mathbf{v}^{\mathrm{lf}} = \mathbf{T} + R\,\mathbf{v}^{\mathrm{mesh}}$). After fitting, capsules are grown against the original (non-manifold) mesh vertices so that vertex coverage is restored against the real collision surface.
...
% ---- F. Coverage-Aware Refinement ----
\subsection{Coverage-Aware Refinement}

The initial capsules undergo a series of refinement passes that jointly improve tightness while preserving or improving measured vertex-sample coverage.

\paragraph{Merging and growth.}
Approximately collinear adjacent capsules (axial dot product $> 0.3$, radius difference $< 15\%$) are merged into a single capsule spanning the full extent. Each vertex is assigned to its nearest capsule by point-to-segment distance, and radii are expanded to cover all assigned vertices, ensuring \Cref{eq:vertex_coverage}.

\paragraph{Endpoint span shrinkage.}
Capsule endpoints are optimized by searching over shrink amounts that minimize volume while maintaining coverage, iterated until convergence.
...
Capsules whose end-spheres lie entirely within another capsule are removed; since capsules are convex, this does not affect coverage.

\paragraph{Budget enforcement.}
If the capsule count exceeds $K_{\max}$, capsules are iteratively removed in order of minimal volume impact, with remaining capsules re-grown after each removal.
