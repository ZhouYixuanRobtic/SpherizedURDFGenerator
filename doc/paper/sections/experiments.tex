...
\textbf{Section count $N$} is the dominant control parameter. Increasing $N$ from 2 to 8 reduces worst-case uncovered distance from 6.69~mm to 1.20~mm (5.6$\times$ improvement) at the cost of raising capsule count from 8 to 11 and $\mathrm{capV/aabb}$ from 1.37 to 1.56. The relationship is not monotonic: the default configuration ($N=4$, 10.23~mm worst) exhibits worse coverage than $N=2$ (6.69~mm). This occurs because the additional section planes cause one link (link3) to be split into two capsules, creating a coverage gap at the split boundary. The radius-growth pass (Section~\ref{sec:method}-F) operates on per-vertex assignments to the nearest capsule axis, and vertices near the split boundary may fall in a narrow region that is not adequately represented by either capsule's assigned vertex set, leaving the gap unclosed. For users who prioritize coverage over capsule count, we recommend $N=2$ (coarser but more conservative, 6.69~mm worst-case) or $N \geq 6$ (finer axial sampling eliminates the boundary gap, $\leq 2.34$~mm worst-case). Addressing this failure mode is a target for future improvement.
...
Per-link breakdowns for all capsule ablation configurations are provided in the artifact directory at \texttt{experiments/per\_link\_metrics.csv}, and in the aggregate comparison at \texttt{artifacts/exp-fr3-mode-comparison/data/fr3\_comparison\_table.md}.
