...
Vertex coverage alone does not guarantee continuous surface coverage. We audited surface coverage for all mode-comparison outputs by sampling 12,000 points per link mesh surface (deterministic seed, area-weighted). Capsule modes achieve negative worst signed distance on all 8 arm links (i.e., perfect surface coverage), while sphere modes leave small surface gaps (worst 1.7~mm for sphere/default, 0.7~mm for sphere/single). Convex mode achieves zero surface gap by construction. Hand and finger links show poor coverage across all analytic modes (64--90~mm gaps, 76--91\% uncovered), indicating that the current presets are tuned for arm-like geometry. Surface and vertex coverage agree on covered/uncovered classification, but vertex-only metrics can miss inter-sphere gaps in sphere-tree modes.
