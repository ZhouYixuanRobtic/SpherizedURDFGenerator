...
The medial-axis sphere tree~\cite{bradshaw2002sphere, mlund_spheretree} constructs an inner approximation whose spheres lie within the original mesh, providing inner (under-approximating) collision estimates.
...
% ---- 5. Tooling Gap -%
While each component---convex decomposition, sphere-tree generation, and capsule fitting---has been addressed individually, to the best of our knowledge prior work does not combine them into a single URDF-to-URDF pipeline with sidecar analytic parameter export, built-in validation metrics, and preset-based reproducible tuning. The MathWorks capsuleApproximation function (R2022b) provides capsule fitting for rigid body trees but is closed-source and not URDF-native. While each of the component algorithms has been studied individually, the contribution of this work is the integration of these algorithms into a single, reproducible URDF-to-URDF pipeline with sidecar primitive export, built-in validation metrics, and preset-based configuration---a combination that, to our knowledge, has not been previously available as an open-source tool.
