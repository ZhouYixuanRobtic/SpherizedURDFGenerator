...
It supports a fixed set of collision primitives---spheres, cylinders, boxes, and meshes---but lacks native capsule elements, requiring capsule-based approximations to be decomposed into cylinders and spheres.
Xacro~\cite{xacro} extends URDF with macro preprocessing but the output remains standard URDF.
Several recent works have proposed alternative description formats such as MJCF~\cite{todorov2012mujoco} and SDFormat~\cite{sdformat}, yet URDF remains the de facto standard for ROS-compatible manipulators.
...
% ---- 5. Geometry-Processing Dependencies -%
Our toolchain builds on several established geometry-processing libraries.
CGAL~\cite{cgal} and libigl~\cite{jacobson2018libigl} provide the convex-hull computation and general mesh processing.
ManifoldPlus~\cite{huang2022manifoldplus} converts non-manifold or open meshes into watertight manifolds, which is a prerequisite for the cross-section slicing in our capsule pipeline.
Eigen~\cite{eigen} provides the linear algebra backbone for PCA, covariance computation, and geometric transformations.
URDF parsing and writing is handled by urdfdom~\cite{urdfdom}.

% ---- 6. Tooling Gap -%
While each of the component algorithms---convex decomposition, sphere-tree generation, and capsule fitting---has been addressed individually in the literature, no existing open-source toolchain integrates them into a single URDF-to-URDF pipeline with sidecar primitive parameter export, validation metrics, and presets for reproducible tuning.
The primary gap our work fills is not a novel fitting algorithm per se but rather the integration of these components into a practical, extensible tool that immediately produces usable URDF output and analytically reusable primitive data.
