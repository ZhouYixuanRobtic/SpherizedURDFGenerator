...
As a minimal integration example, the sidecar can be consumed in a few lines of Python:
\begin{verbatim}
import json

with open("output_sidecar.json") as f:
    sidecar = json.load(f)

for link_name, capsules in sidecar["capsules"].items():
    for cap in capsules:
        p0, p1, r = cap["p0"], cap["p1"], cap["radius"]
        # p0, p1 are 3D endpoints; r is the capsule radius
        # Evaluate point-to-capsule distance:
        # d = max(0, distance_to_segment(query, p0, p1) - r)
\end{verbatim}
...
% ---- C7: Known Limitations ----
\subsection{Known Limitations}

The current release has several known limitations that users should be aware of:

\begin{enumerate}
    \item \textbf{Non-monotonic $N=4$ coverage.} Increasing section count from $N=2$ to $N=4$ can introduce a coverage gap at the split boundary between two capsules (Section~\ref{sec:experiments}-F). Users are advised to use $N=2$ for conservative coverage or $N \geq 6$ for tight fits.

    \item \textbf{PCA axis assumption.} The capsule pipeline assumes each link has a dominant principal axis (Section~\ref{sec:discussion}). Highly branched, flat, or spherical links may produce poor capsule fits; convex or sphere modes are better choices in these cases.

    \item \textbf{Splitting not triggered on typical arm links.} The inflation-based splitting pass (Section~\ref{sec:method}-F, Section~\ref{sec:metrics}-D) does not activate for the Franka FR3 under default parameters. Users with irregular link geometries may need to tune $\rho_{\max}$ and $\delta_{\min}$.

    \item \textbf{Single-robot evaluation.} The current experiments are limited to the Franka FR3 arm (Section~\ref{sec:experiments}-B). Performance on other robot morphologies has not been characterized.

    \item \textbf{JSON sidecar requires user integration.} The canonical capsule parameters in the sidecar are not consumed by standard URDF simulators (Section~\ref{sec:discussion}). Users must write a small integration layer to use them directly.

    \item \textbf{No native Xacro support.} The toolchain accepts only plain URDF; Xacro files must be preprocessed externally (Section~\ref{sec:overview}-A).
\end{enumerate}
